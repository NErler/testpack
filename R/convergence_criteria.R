...
GR_crit <- function(object, confidence = 0.95, transform = FALSE,
  ...
  if (!inherits(object, "testpack"))
    errormsg('Object must be of class "testpack".')
...
#' 'testpack' model, combining the samples from all MCMC chains.
...
MC_error <- function(x, subset = NULL, exclude_chains = NULL,
  ...
  if (!inherits(x, "testpack"))
    errormsg("%s must be of class %s.", dQuote("x"), sQuote("testpack"))
